...
impl RTTWindow {
    ...
    fn clear_old_hist(&mut self, now: u64) {
        ...
        while self.times.len() > 1 && self.times.front().unwrap() < &(now - self.max_time) {
    ...
    pub fn did_base_rtt_change(&mut self) -> bool {
        let b = self.base_rtt_changed;
        self.base_rtt_changed = false;
        b
    }
    ...
    pub fn tcp_detected(&mut self) -> bool {
        ...
        for i in 0..(self.rtts.len()) {
            if self.times[i] > self.times.back().unwrap() - self.srtt as u64 * 10 {
